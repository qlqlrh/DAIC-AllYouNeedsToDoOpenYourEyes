...
## 화면에서 localhost:3000 에러가 뜰 경우 -> 환경변수가 제대로 주입이 되지 않은 것 , STTRecorder와 Page .tsx 에 있는 환경변수 상태를 .env.local 파일을 참고하여 하드코딩으로 진행
```
...
### ⚠️ 참고 사항
백엔드는 API 키 문제로 기본적으로 비활성화되어 있습니다.

키 파일을 resource 폴더 하위에 추가하면 정상 동작합니다.

키 파일은 별도로 이메일로 제출하려 합니다.

백엔드 배포는 완료하였지만 RAG_LANGCHAIN 의 배포에서 모델 용량이 크기 때문에 프리티어 환경에서는 배포가 끊겨, 제출용 배포는 하지 못하였습니다.
 (추후 시연 시는 잠시 사용 가능한 서버 배포로 시연 예정.)
```bash
# javaSpringBoot- 백앤드 서버 구동
./gradlew clean build
--> 빌드 진행 (SDK 21 , java 17)

java -jar build/libs/(jar 파일 이름 )-SNAPSHOT.jar
--> 실행 (인텔리제이가 없다면 , 있다면 어플리케이션 실행으로 진행해도 무관)

# 의존성 에러시 (몽고 디비 / redis 는 없어도 무관 )
```
